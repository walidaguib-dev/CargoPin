...
import { deleteShipment } from "@/lib/shipments/api";

import type { Shipment } from "@/lib/shipments/types";

interface DeleteShipmentPopupProps {
  shipment: Shipment | null;
  onOpenChange: (open: boolean) => void;
  onDeleted?: (id: number) => void;
}

export function DeleteShipmentPopup({
  shipment,
  onOpenChange,
  onDeleted,
}: DeleteShipmentPopupProps) {
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-105 rounded-2xl p-6 sm:rounded-2xl">
        ...
        <p className="mt-4 text-[14px] leading-relaxed text-[#374151]">
          ...
          <span className="font-semibold text-[#0F172A]">{shipment?.clientName}</span>?

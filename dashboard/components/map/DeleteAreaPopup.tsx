...
export interface DeleteAreaTarget {
  id: number;
  name: string;
}

interface DeleteAreaPopupProps {
  target: DeleteAreaTarget | null;
  onOpenChange: (open: boolean) => void;
  onDeleted: () => void;
}

export function DeleteAreaPopup({ target, onOpenChange, onDeleted }: DeleteAreaPopupProps) {
  ...
  const handleConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteArea(target.id, accessToken);
